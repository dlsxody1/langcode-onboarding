...
> 📖 **실행하지 않고 읽기만 할 거라면:**
> 이 실습에서 나온 **측정 결과와 실패 사례는 전부 본문 문서에 옮겨 놓았다.**
> - 청킹 설정별 성능 실측 → [`03-rag/02-chunking-embedding.md`](../../03-rag/02-chunking-embedding.md) **3-2절**
> - 의미 검색이 실패하는 실물 → 같은 문서 **5-2절**
> - 평가셋을 잘못 만든 실제 사례 → [`03-rag/03-retrieval-quality.md`](../../03-rag/03-retrieval-quality.md) **4절**
>
> **그 세 곳만 읽어도 이 실습의 결론은 다 얻는다.** 아래는 직접 돌려볼 때의 안내다.
> 다만 `minirag.py` 는 한 번 훑어볼 가치가 있다 — RAG 전체가 350줄이라는 걸 눈으로 보는 것 자체가 의미 있다.
...
| --- | ---: | ---: | ---: | ---: |
| fixed 800 / overlap 80 | 5 | 0.600 | 1.000 | 0.780 |
| fixed 400 / overlap 40 | 9 | 0.560 | 0.920 | 0.753 |
| heading (구조 기반) | 21 | 0.600 | 0.800 | 0.736 |
| fixed 150 / overlap 50 | 30 | — | 0.760 | 0.667 |
| **fixed 150 / overlap 0** | 21 | **0.440** | **0.720** | **0.613** |

**모델도 프롬프트도 안 바꿨는데 Recall@1 이 0.600 → 0.440 으로 떨어진다.**
조항이 중간에서 잘려 답이 두 청크에 나뉘기 때문이다.

⚠️ **단, 이 표의 1등(800자)을 그대로 믿으면 안 된다.** 청크가 5개뿐이라 `k=3` 이면
전체의 60% 를 통째로 넣는 것 — 검색이랄 게 없다. **코퍼스가 6KB 라서 생긴 인공물이다.**
그래서 `Recall@1` 과 `MRR` 로 봐야 하고, 거기서는 구조 기반(21청크)이 800자(5청크)와 동점이다.

> 자세한 해설은 [`03-rag/02-chunking-embedding.md` 3-2절](../../03-rag/02-chunking-embedding.md) 에 있다.

**해볼 것:** `--overlap` 을 0 → 50 → 80 으로 올려봐라. **좋아지지 않고 나빠진다.**
(0.880 → 0.840 → 0.800, 청크 수는 21 → 30 → 43)
중복 청크가 top-k 자리를 차지해서 정작 필요한 청크가 밀려나기 때문이다.

### 과제 2 — TF-IDF 로는 못 찾는 질문

```bash
python3 minirag.py eval
```

`heading` 전략에서 **딱 하나가 모든 설정에서 실패**한다.

```
[vector] 놓친 질문 1개:
   - 새로 들어온 사람도 집에서 일할 수 있나요?
```

정답은 `제42조 — 신규 입사자는 입사 후 3개월간 재택근무를 신청할 수 없다` 인데,
실제로 가져오는 건 전혀 상관없는 휴가 문서들이다.
...
**TF-IDF 는 글자가 겹쳐야 가깝다. 의미를 모른다.** 이게 진짜 임베딩 모델이 필요한 이유의 전부다.
(1등 점수가 0.1140 으로 현저히 낮은 것도 신호다 — "못 찾았다"를 점수로 감지할 수 있다)

키가 있다면 차이를 바로 볼 수 있다:
...
키가 없으면 실행하지 말고 **왜 실패하는지만 이해하고 넘어가라.** 그게 핵심이다.
